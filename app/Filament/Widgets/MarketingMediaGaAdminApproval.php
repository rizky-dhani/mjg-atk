...
class MarketingMediaGaAdminApproval extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $usagesCount = MarketingMediaStockUsage::where('status', MarketingMediaStockUsage::STATUS_APPROVED_BY_HEAD)
        ->where('division_id', $user->division_id)
        ->count();
            ...
            Stat::make('Waiting for Approval', $requestsCount)
                ->url(
                    route('filament.dashboard.resources.office-stationery-stock-requests.index', [
                        'tableFilters[status][value]' => MarketingMediaStockRequest::STATUS_APPROVED_BY_HEAD
                    ])
                )
            ...
            Stat::make('Waiting for Approval', $usagesCount)
                ->url(
                    route('filament.dashboard.resources.office-stationery-stock-usages.index', [
                        'tableFilters[status][value]' => MarketingMediaStockUsage::STATUS_APPROVED_BY_HEAD
                    ])
                )
    ...
    public static function canView(): bool
    {
        ...
        // Check if user is GA Admin
        return $user->division && $user->hasRole('Admin') && $user->division?->initial === 'GA';
    }
}
